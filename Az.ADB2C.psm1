...
function Get-BrowserInfo(
    [Parameter(Mandatory=$false)][Boolean]$Chrome = $True,
    [Parameter(Mandatory=$false)][Boolean]$Edge = $False,
    [Parameter(Mandatory=$false)][Boolean]$Firefox = $False,
    [Parameter(Mandatory=$false)][Boolean]$Incognito = $False,
    [Parameter(Mandatory=$false)][Boolean]$NewWindow = $False
)
{
    $pgm = "chrome.exe"
...
<#
.SYNOPSIS
    Downloads the Azure AD B2C Starer 
...
(
    ...
    [Parameter(Mandatory=$false)][switch]$Clean = $False            # if to "clean" the policies and revert to "itthingsb2c.onmicrosoft.com" etc
    )
{
...
.PARAMETER username
    Name of user. Default is "graphexplorer". This will make the UPN "graphexplorer@itthingsb2c.onmicrosoft.com"
...
<ClaimsProvider>
  ...
  <TechnicalProfiles>
    ...
    <TechnicalProfile Id="$($samleTpId)">
      ...
      <Metadata>
        <!-- The issuer contains the policy name; it should be the same name as configured in the relying party application. B2C_1A_SignupOrSignin_SAML is used below. -->
...
#>
function Connect-AzADB2CDevicelogin2 { #Connect-AzureDevicelogin {
    [cmdletbinding()]
    param( 
        [Parameter()]
...
.PARAMETER TenantID
    Specify the tenant name (itthingsb2c.onmicrosoft.com) or the id (guid) for the tenant
...
.EXAMPLE
    $tokens = Connect-AzADB2CDevicelogin -TenantName "itthingsb2c.onmicrosoft.com" -Scope "User.Read.All" 

.EXAMPLE
    $tokens = Connect-AzADB2CDevicelogin -TenantName "itthingsb2c.onmicrosoft.com" -Scope "Directory.ReadWrite.All" 
#>
...
.DESCRIPTION
    converts tenant ids (guids) to real names, like itthingsb2c.onmicrosoft.com
...
Export-ModuleMember -function Connect-AzADB2CDevicelogin2
...
Export-ModuleMember -function Get-AzADB2CPolicy
...
Export-ModuleMember -function Connect-AzureDevicelogin
Export-ModuleMember -function Start-Browser
Export-ModuleMember -function Get-BrowserInfo